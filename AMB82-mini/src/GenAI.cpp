...
// Server: Google AI Studio
String GenAI::ollamavision(String model, String message, uint32_t img_addr, uint32_t img_len, WiFiSSLClient client)
{
    const char *myDomain = "192.168.3.246";
    String getResponse = "", Feedback = "";
    Serial.println("Connect to " + String(myDomain));
    if (client.connect(myDomain, 11434)) {
        ...
        String Data = "{\"data\": {\"model\": \"granite3.2-vision\", \"prompt\": \"" + message + "\", \"stream\": {\"false,\"images\":[\"" + imageFile + "\"]}}";

        client.println("POST /api/generate HTTP/1.1");
        client.println("Host: " + String(myDomain));
        client.println("Content-Type: application/json; charset=utf-8");
        client.println("Content-Length: " + String(Data.length()));
        client.println("Connection: close");
